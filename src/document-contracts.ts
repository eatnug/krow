...
function traceLinks(content: string, ref: string, kind: KrowDocumentKind): TraceLink[] {
  ...
  for (const match of content.matchAll(/^PRD ID:\s*(PRD-\d+)\s*$/gim)) {
...
export function findRelatedDocuments(
  ...
  return {
    related,
    approvalGaps: related.filter((doc) => (doc.kind === "prd" || doc.kind === "plan") && doc.approval.status !== "approved"),
  };
}
...
export function approvalPromptsForDocuments(documents: ApprovalTargetDocument[]): DecisionPrompt[] {
  return documents
    ...
    .map((document) => {
      const kind = document.kind as "prd" | "plan";
      return {
        ...
        options: [
          {
            id: "approve",
            label: "Approve",
            description: "Record approval in workflow decision history and continue toward execution.",
          },
          {
            id: "revise",
            label: "Revise",
            description: "Return to clarify so the document or plan can be revised before implementation.",
          },
          {
            id: "stop",
            label: "Stop",
...
export function unsatisfiedApprovalDocuments(
  ...
  return documents.filter(
    (document) =>
      (document.kind === "prd" || document.kind === "plan") &&
