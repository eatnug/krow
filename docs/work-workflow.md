...
compound reusable language into System Documents that describe behavior, responsibilities, rules, states, boundaries, and code references
...
When new reusable meaning is needed for the current work, `plan` updates the actual `.krow/system` docs before implementation depends on it. Glossary names the concepts, System Map routes to the relevant area, and System Documents carry the durable behavior description with references. The plan-review AskAction points the user at those changed docs; rejection sends the workflow back to planning so the same docs can be revised.
