...
- keep the always-loaded instruction lean and move workflows into installed host skills plus coded runtime instructions
...
### Layer B: Runtime Phase Instructions

Runtime phase instructions are generated by the local runtime. They inherit the global contract but sharpen it for one current phase and one current unit.
...
The runtime emits phase instruction refs such as `krow://phase/clarify`; phase behavior is generated by runtime code.

### Layer C: Skills

Skills hold repeatable workflows that do not belong in every session.

User-facing host skills:

- `$work`: actionable engineering work that may edit source code inside the approved task scope
- `$check`: repository alignment check that reads the project and writes only `.krow` files

The intended structure is:

- the host exposes `$work` and `$check` using its own registration mechanism
- both triggers map into the same local runtime
- state transitions stay in the CLI/runtime rather than hidden chat convention
...
The engine should not depend on instructions alone for control.
...
This keeps the model inside coded rails rather than asking instructions to self-police every turn.
...
This mode gathers enough anchor evidence for the first real unit to enter `clarify`.

### Mode 3: Relay Run

Use when the task has multiple file or module surfaces, parallelizable subtasks, or a meaningful internal retry loop.

This mode creates multiple bounded units with explicit ownership and relay files.

### Mode 4: Finish Loop

Use when the user's intent is "keep going until it is actually done."

This mode keeps the phase loop running until verification passes, a blocker is recorded, or a retry limit is reached.

## 4A. Entry Surface

To keep the system easy to adopt, expose only two broad surfaces by default:

- `$work`
- `$check`

Everything else remains an internal local control command.
...
Use an anchor gate when the request lacks:

- a concrete file or symbol target
- clear acceptance criteria
- a narrow execution edge

The output is not a fixed step ladder. It is simply enough structure for the first real unit to enter `clarify`.

## 11. Multi-Unit Work

Use multiple bounded units when work spans multiple task surfaces.
...
The completion loop is a runtime posture, not a separate pipeline.
...
- output contracts for `clarify`, `execute`, `verify`, and optional `capture`
- signal contracts for `run`, `gate`, `done`, and `fault`
- a transition table
...
- local control commands for route/intake/check/check-apply/documents/review/start/status/next/stop
